...
feature 'headhunter consults a proposal' do
    context 'from proposal' do 
        ...
        scenario 'and not saw a closed proposal' do
            headhunter = Headhunter.create!(email: 'headhunter@teste.com',
                                        password: '123teste')
            ...
            proposal = Proposal.create!(start_date: Date.current.next_day(15), limit_feedback_date: Date.current.next_day(7), status: :closed,
                                        salary: 2600, benefits: 'VT, VR, convenio medico e seguro de vida', registered_id: registered.id)
            ...
            expect(page).not_to have_content("Proposta enviada para: #{profile.name}")
